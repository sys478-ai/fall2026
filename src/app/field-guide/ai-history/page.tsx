...
export const metadata: Metadata = {
  ...
  description: 'An interactive timeline of AI from Turing to today, linking key moments to field guide recognition cards.',
};
...
function getTimelineEntries(): TimelineEntry[] {
  ...
  const examplePosts = getAllPosts('examples').filter(
    p => !!(p as PostWithExtras).show_in_timeline
  );

  return [
    ...historyPosts.map(p => toEntry(p)),
    ...examplePosts.map(p => toEntry(p, p.id)),
  ].sort((a, b) => parseInt(a.year, 10) - parseInt(b.year, 10));
}

function CardChip({ card }: { card: TimelineCard }) {
  const base =
    'inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium no-underline transition-colors';
  if (card.href) {
    return (
      <Link
        href={card.href}
  ...
  return (
    <span className={`${base} bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-500`}>
      {card.label}
    </span>
  );
}
      ...
      header={
        <header className="border-y border-violet-200 bg-violet-50 px-4 py-16 dark:border-violet-900 dark:bg-violet-950/30 md:px-16">
          ...
          <p className="mb-0 mt-5 max-w-4xl text-lg leading-8 text-gray-700 dark:text-gray-300">
            Key moments in AI history, linked to field guide recognition cards. Purple chips link to published cards; gray chips are connections to cards not yet published. Case study entries link to full example cards.
          </p>
        </header>
      }
    >
            ...
            {entries.map((entry, i) => (
              <div key={i} className="relative pb-12 last:pb-0">
                <div
                  ...
                  aria-hidden="true"
                />
                <div className="mb-1 text-sm font-bold text-violet-700 dark:text-violet-300">
                  {entry.year}
                </div>
                <h2 className="m-0! mb-2 text-xl font-semibold leading-snug text-gray-950 dark:text-gray-50">
                  {entry.title}
                </h2>
                <p className="mb-3 text-base leading-7 text-gray-700 dark:text-gray-300">
                  {entry.description}
                </p>
                {entry.cards.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {entry.cards.map((card, j) => (
                      <CardChip key={j} card={card} />
                    ))}
                  </div>
                )}
                {entry.contested && (
                  <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 dark:border-amber-900/50 dark:bg-amber-950/20">
                ...
                {entry.exampleSlug && (
                  <div className="mt-3">
                    <Link
                      ...
                      className="inline-flex items-center gap-1 rounded-full bg-violet-50 px-3 py-1 text-xs font-semibold text-violet-700 no-underline ring-1 ring-violet-200 transition-colors hover:bg-violet-100 dark:bg-violet-950/30 dark:text-violet-300 dark:ring-violet-800 dark:hover:bg-violet-900/40"
                    >
                      View full example →
                    </Link>
                  </div>
